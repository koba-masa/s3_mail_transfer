...
def lambda_handler(event:, context:)
  ...
  original_mail = OriginalMail.new(custom_event.message_id)
  logger.debug("Original Raw Email: #{original_mail.body}")

  edited_raw_mail = edit_raw_mail(original_mail)
  logger.debug("Edited Raw Email: #{edited_raw_mail}")

  send(edited_raw_mail)
